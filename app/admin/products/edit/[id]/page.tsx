...
export default function EditProduct() {
  ...
  const id = params.id as string; // This is the productID
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [productRes, schemaRes] = await Promise.all([
  ...
  if (loading) return <div className="p-8 text-center"><span className="loading loading-spinner text-pink-500 loading-lg"></span></div>;

  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      <div className="flex items-center gap-4 border-b border-zinc-800 pb-4">
        <Link href="/admin/products" className="btn btn-circle btn-ghost text-zinc-400 hover:text-white">
          <ArrowLeft size={24} />
        </Link>
        <div>
          <h2 className="text-3xl font-black text-white tracking-tight">Edit Product</h2>
          <p className="text-zinc-400 font-medium">Modify standard and dynamic fields.</p>
        </div>
      </div>
          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-zinc-300 ml-1">Product ID</label>
                <input type="text" className="w-full bg-zinc-950/50 border border-zinc-800 text-white rounded-xl p-3 outline-none opacity-50 cursor-not-allowed" disabled
